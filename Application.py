...
class Application:
	...
	# Function used to make call web crawler and return as html elements
	def getCrawlerDataAndQuery(url, host, tableName):
		try:
...
<li><a href="/wiki/ZPL_(programming_language)" title="ZPL (programming language)">ZPL</a></li>
			...
			crawler = WikiCrawler(url, host, sampleData)
			elements = crawler.urlRequester()
			if elements :
				return crawler.makeValueList(elements)
			else:
				raise Exception('Wikipedia data not received')
	
		except Exception as ex:
			print('Exception at: ',ex);
		
	# Function to create table if needs and insert data on table
	def makeDataInsert(query,listOfData, isNeedtable, tableString):
		connector = DBConnector('',"SampleCrawlerDB.db")
		conn = connector.create()
		dbCmt = DBManipulation(conn)
		if isNeedtable :
			dbCmt.createTable(tableString)
		dbCmt.queryExecutor(query, listOfData)

if __name__ == "__main__" :
	isNeedTable = sys.argv[1]
	listOfData = Application.getCrawlerDataAndQuery("https://en.wikipedia.org/wiki/List_of_programming_languages","https://en.wikipedia.org", "computer_langues")
	tableString = "CREATE TABLE computer_langues(   ID INTEGER PRIMARY KEY   AUTOINCREMENT,   name           TEXT,   url            TEXT,   host        TEXT)"
	query = "INSERT INTO computer_langues (name, url, host) VALUES(?,?,?)"
	if isNeedTable == '1':
		Application.makeDataInsert(query,listOfData, True, tableString)
	else:
		Application.makeDataInsert(query,listOfData, False, tableString)
